import requests, json, threading, os, tempfile, pandas as pd, numpy as np  # who the fuck uses pandas over polars?
...
def get_historical_data(symbol):
	...
	df = pd.DataFrame(raw_data, columns=["open_time", "open", "high", "low", "close", "volume", "close_time", "quote_asset_volume", "trades", "taker_buy_base", "taker_buy_quote", "ignore"])
...
def plot_market_structure(symbols):
	...
	def add_trace(*args):
		...
		fig.add_trace(go.Scatter(x=normalized_df.index, y=y, mode="lines", name=name, line=line, showlegend=legend))
